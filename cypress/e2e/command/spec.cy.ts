describe('Command', () => {
  ...
  it('should handle Tools dropdown keyboard navigation', () => {
    ...
    cy.get('[id^="command-"][id$="icture"], [id^="command-"][id$="anvas"], [id^="command-"][id$="ticky"]').should('exist');
  });
  ...
  it('should support Shift+Enter for newlines without submitting', () => {
    cy.visit('/');
    cy.get('#chat-input').should('exist');
    cy.wait(300);
    ...
    cy.get('.step').should('have.length', 0); // nothing submitted yet
    cy.get('#chat-input').invoke('val').should('contain', 'Line 1\nLine 2');
  ...
  it('should treat pasted content as plain text (no HTML injection) and keep newlines', () => {
    cy.visit('/');
    cy.get('#chat-input').should('exist');
    cy.wait(300);

    cy.get('#chat-input').then(($el) => {
      const el = $el[0] as HTMLTextAreaElement;
      cy.window().then((win) => {
        const data = new DataTransfer();
        const pasted = 'Hello <b>world</b>\n<script>alert(1)</script>\nLine 3';
        data.setData('text/plain', pasted);

        // Fire a native paste event; our component listens on the DOM node
        const evt = new (win as any).ClipboardEvent('paste', { clipboardData: data });
        el.dispatchEvent(evt);
      });
    });

    cy.get('#chat-input').should('have.value', 'Hello <b>world</b>\n<script>alert(1)</script>\nLine 3');

    // Submit and ensure no command was attached and content didn't break submission
    cy.get('#chat-input').type('{enter}');
    cy.get('.step').last().should('contain', 'Command:');
  });
